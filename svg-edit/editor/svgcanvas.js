...
// 					console.log('s',[start_x, start_y]);y
...
// 					}
	...
	var mouseUp = function(evt)
	{
		...
		switch (current_mode)
		{
			...
			case "magic":
				if (rubberBox != null) {
					// Use the raw, un-zoomed mouse coordinates to build the selection box.
					var selectionBox = {
						x: Math.min(r_start_x, real_x),
						y: Math.min(r_start_y, real_y),
						width: Math.abs(real_x - r_start_x),
					...
					// Call the function that now handles both creation and finalization.
					if (svgCanvas.drawOcrRectsInSelection) {
						svgCanvas.drawOcrRectsInSelection(selectionBox);
					}
					rubberBox.setAttribute("display", "none");
					curBBoxes = [];
				}
