...
# Path to the bundled site-packages directory
bundled_site_packages = os.path.join(addon_path, "vendor")
...
# --- Helper functions for EasyOCR and Bounding Box Merging ---

def get_box_coords(bbox):
    """
    Extracts the minimum and maximum x and y coordinates from a bounding box.
    A bounding box is expected to be a list of four [x, y] points.
    """
    x_coords = [p[0] for p in bbox]
    y_coords = [p[1] for p in bbox]
    return min(x_coords), min(y_coords), max(x_coords), max(y_coords)

def do_boxes_overlap(bbox1, bbox2, overlap_threshold=0):
    """
    Checks if two bounding boxes overlap based on a given threshold.
    """
    x1_min, y1_min, x1_max, y1_max = get_box_coords(bbox1)
    x2_min, y2_min, x2_max, y2_max = get_box_coords(bbox2)

    horizontal_overlap_val = max(0, min(x1_max, x2_max) - max(x1_min, x2_min))
    vertical_overlap_val = max(0, min(y1_max, y2_max) - max(y1_min, y2_min))

    return horizontal_overlap_val > overlap_threshold and vertical_overlap_val > overlap_threshold
...
class ImgOccAdd(object):
    def __init__(self, editor, origin, oldimg=None):
        ...
        loadConfig(self)
        # Initialize EasyOCR reader here, as it's only needed once
        # Set `gpu=False` if you don't have a GPU or encounter issues.
        try:
            self.reader = easyocr.Reader(['en'], gpu=True) 
        except Exception as e:
            # Handle cases where EasyOCR might fail to initialize (e.g., missing dependencies)
            showWarning(f"Failed to initialize EasyOCR: {e}. Automated text cover might not work.")
            self.reader = None
    ...
    def callImgOccEdit(self, width, height):
        ...
        dialog = ImgOccEdit(self, self.ed.parentWindow, image_path=self.image_path, ocr_reader=self.reader)
        ...
        dialog.setWindowState(Qt.WindowState.WindowMaximized | Qt.WindowState.WindowActive)
    
    def onAutomatedCover(self):
        """
        Reads the current image, finds text regions using EasyOCR,
        merges overlapping boxes, and instructs the WebView to draw rectangles over them.
        """
        try:
            ...
            if self.reader is None:
                showWarning("EasyOCR reader not initialized. Cannot perform automated text cover.")
                return

            img = cv2.imread(image_path)
            if img is None:
                showWarning(f"Could not load image from {image_path}. Please check the path.")
                return

            # Convert image to RGB for EasyOCR
            image_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

            # Detect text using EasyOCR (returns (bbox, text, confidence) tuples)
            results = self.reader.readtext(image_rgb)

            if not results:
                tooltip("No text detected.")
                return

            # Extract bounding boxes from EasyOCR results
            # EasyOCR's bounding box is a list of 4 points: [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]
            # We'll also filter by confidence, similar to your original Tesseract logic.
            # EasyOCR confidence is a float between 0 and 1.
            bounding_boxes = [r[0] for r in results if r[2] > 0.60] # Adjust confidence threshold (0.60 corresponds to > 60 in Tesseract's 0-100 scale)

            # --- Merging Logic ---
            merged_boxes = []
            unmerged_boxes = list(bounding_boxes)

            while unmerged_boxes:
                current_cluster_bbox = unmerged_boxes.pop(0)

                i = 0
                while i < len(unmerged_boxes):
                    if do_boxes_overlap(current_cluster_bbox, unmerged_boxes[i]):
                        x_min_curr, y_min_curr, x_max_curr, y_max_curr = get_box_coords(current_cluster_bbox)
                        x_min_other, y_min_other, x_max_other, y_max_other = get_box_coords(unmerged_boxes[i])
                        ...
                        # Update current_cluster_bbox with the new merged coordinates
                        # EasyOCR's bbox is 4 points, so reconstruct similarly.
                        current_cluster_bbox = [[new_min_x, new_min_y], [new_max_x, new_min_y],
                                                [new_max_x, new_max_y], [new_min_x, new_max_y]]

                        unmerged_boxes.pop(i)
                    else:
                        i += 1
            ...
            for bbox in merged_boxes:
                # Convert the merged bounding box (4 points) into x, y, width, height
                x_min, y_min, x_max, y_max = get_box_coords(bbox)
                x = int(x_min)
                y = int(y_min)
                w = int(x_max - x_min)
                h = int(y_max - y_min)
                rects_to_draw.append({"x": x, "y": y, "w": w, "h": h})

            if not rects_to_draw:
                tooltip("No text detected after merging and filtering.")
                return

            # This line sends the list of rectangles to the editor to be drawn.
            self.imgoccedit.drawOcrRects(rects_to_draw)
            tooltip(f"Added {len(rects_to_draw)} occlusion masks.")

        except Exception as e:
            # Catch general exceptions for robustness
            showWarning(f"An error occurred during text recognition: {e}\n\nPlease ensure EasyOCR and its dependencies are correctly installed. You might need to run 'pip install easyocr opencv-python numpy'.")
